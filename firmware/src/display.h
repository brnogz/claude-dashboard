...
void DashboardDisplay::showConnectionLost(TFT_eSPI& tft) {
    tft.fillRect(SCREEN_W - 60, HEADER_Y + 2, 58, HEADER_H - 4, CLR_HEADER_BG);
    tft.setTextColor(CLR_RED, CLR_HEADER_BG);
    tft.setTextFont(1);
    tft.setTextDatum(TR_DATUM);
    tft.drawString("NO LINK", SCREEN_W - 4, HEADER_Y + 8);
    tft.setTextDatum(TL_DATUM);
}
...
void DashboardDisplay::drawStatic(TFT_eSPI& tft) {
    ...
    tft.drawString("CLAUDE", 6, HEADER_Y + 10);
    tft.setTextColor(CLR_TEXT, CLR_HEADER_BG);
    tft.drawString("CODE", 104, HEADER_Y + 10);
...
void DashboardDisplay::drawHeader(TFT_eSPI& tft, const DashboardData& data) {
    ...
    // Clear right portion of header
    tft.fillRect(170, HEADER_Y, SCREEN_W - 170, HEADER_H, CLR_HEADER_BG);

    char buf[32];
    tft.setTextFont(2);
    tft.setTextDatum(TR_DATUM);

    // "[MAX] 8 active" — right-aligned, Font 2
    char countBuf[16];
    snprintf(countBuf, sizeof(countBuf), " %d active", data.activeSessions);
    tft.setTextColor(CLR_TEXT_DIM, CLR_HEADER_BG);
    tft.drawString(countBuf, SCREEN_W - 4, HEADER_Y + 14);

    int countW = tft.textWidth(countBuf);
    snprintf(buf, sizeof(buf), "[%s]", data.plan);
    for (char* p = buf; *p; p++) *p = toupper(*p);
    tft.setTextColor(CLR_GREEN, CLR_HEADER_BG);
    tft.drawString(buf, SCREEN_W - 4 - countW, HEADER_Y + 14);
...
void DashboardDisplay::drawChart(TFT_eSPI& tft, const DashboardData& data) {
    ...
    // Clear entire chart area
    tft.fillRect(0, CHART_Y, SCREEN_W, CHART_H, CLR_BG);
    ...
    for (int i = 0; i < 7; i++) {
        ...
        tft.setTextFont(1);
        tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
        int labelX = chartLeft + i * barSpacing + barSpacing / 2 - 5;
        tft.drawString(data.weekLabels[i], labelX, CHART_Y + CHART_H - 10);
    }
}

void DashboardDisplay::drawFooter(TFT_eSPI& tft, const DashboardData& data) {
    if (!_firstDraw && data.usagePct == _prev.usagePct &&
        data.windowUsed == _prev.windowUsed) return;
    ...
    // "123/900 msgs"
    tft.setTextColor(CLR_TEXT, CLR_BG);
    snprintf(buf, sizeof(buf), "%d/%d msgs", data.windowUsed, data.windowLimit);
    tft.drawString(buf, 6, FOOTER_Y + 2);

    // Progress bar
    int barX = 130;
    int barW = SCREEN_W - 180;
    ...
    tft.setTextColor(CLR_TEXT_DIM, CLR_BG);
    tft.setTextDatum(TR_DATUM);
    tft.drawString("(5h)", SCREEN_W - 4, FOOTER_Y + 2);
    tft.setTextDatum(TL_DATUM);
}
